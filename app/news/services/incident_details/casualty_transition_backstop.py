...
_KEYWORD_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    (
        "توفي واحد من الجرحى",
        re.compile(
            r"(?:بقي.{0,30}(?:جريح|جرحي).{0,40}توف[ىي] واحد(?:ا)?"
            r"|توف[ىي] واحد(?:ا)? من (?:الجرحى|الجرحي|جرحى|جرحي|المصابين))"
        ),
    ),
    (
        "أحد الجرحى استشهد",
        re.compile(r"احد (?:جريحي|الجرحى|المصابين).{0,40}(?:استشهد|توف[ىي]|فارق)"),
    ),
    (
